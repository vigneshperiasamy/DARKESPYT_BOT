...
@user_admin
@loggable
def unmute_all(bot: Bot, update: Update, args: List[str]) -> str:
    chat = update.effective_chat  # type: Optional[Chat]
    user = update.effective_user  # type: Optional[User]
    message = update.effective_message  # type: Optional[Message]
    ...
    user_id = None
    if user_id is None:
        message.reply_text("There's No Any user was already have been muted to be Unmute..")
        return ""
...
*Admin only:*
 ...
 - /unmute_all : unmutes all the muted users.
"""
...
UNMUTE_ALL_HANDLER = CommandHandler("unmute_all", unmute_all, pass_args=True, filters=Filters.group)
TEMPMUTE_HANDLER = CommandHandler(["tmute", "tempmute"], temp_mute, pass_args=True, filters=Filters.group)

dispatcher.add_handler(MUTE_HANDLER)
dispatcher.add_handler(UNMUTE_HANDLER)
dispatcher.add_handler(UNMUTE_ALL_HANDLER)  
